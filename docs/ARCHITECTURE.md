...
| `resume` | — | `{}` |
| `setTorch` | `{ on: bool }` | `{}` |
| `captureAndRectify` | — | `{ path: String, widthPx: Int, heightPx: Int, quad: List<{x, y}>, quadSource, uri, width, height }` — `quad` is the final still-space quad used for the warp (normalized, top-left origin); `quadSource` is `refined` (on-still re-detection accepted at IoU ≥ 0.8) or `preview` (mapped preview-quad fallback). iOS emits `quadSource`; Android does not yet (Sprint 3). **Platform divergence (geometry-equivalent, flagged intentionally):** iOS applies `CIPerspectiveCorrection` to the last smoothed quad and writes a JPEG to `NSTemporaryDirectory()`; Android (Sprint 4) decodes the full-res still, scales the normalized quad into pixel space, and routes through the shared hand-rolled native warp (`supy_core_warp` → JNI `nativeWarpPerspective`), then `PageReencoder.reencodeBitmap`. Routing iOS through the shared native warp for pixel-identical output is deferred (optional per the Sprint 4 plan). When no quad is detected, both platforms return `captureUnsupported` so the Dart widget falls back to `captureFullFrame`. Errors: `captureUnsupported`, `captureFailed`, `unknown`. |
